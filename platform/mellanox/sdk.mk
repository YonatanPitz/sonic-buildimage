...
MLNX_SDK_RDEBS += $(IPROUTE2_MLNX) $(SX_COMPLIB) $(SX_EXAMPLES) \
                  $(SX_GEN_UTILS) $(SX_SCEW) $(WJH_LIBS) $(SX_ACL_HELPER)
...
MLNX_SDK_BUILD_RDEBS = $(APPLIBS) $(SXD_LIBS)
...
$(eval $(foreach deb,$(MLNX_SDK_BUILD_RDEBS ),$(call make_path,$(deb))))
$(eval $(foreach deb,$(PYTHON_SDK_API) $(SX_KERNEL) $(SX_KERNEL_DEV),$(call make_path,$(deb))))

SONIC_MAKE_DEBS += $(SX_KERNEL)

ifeq ($(SDK_FROM_SRC), y)
SONIC_MAKE_DEBS += $(MLNX_SDK_RDEBS) $(PYTHON_SDK_API) $(MLNX_SDK_BUILD_RDEBS)
else
SONIC_COPY_DEBS += $(MLNX_SDK_RDEBS)
SONIC_MAKE_DEBS  += $(MLNX_SDK_BUILD_RDEBS) $(PYTHON_SDK_API)
endif

SONIC_STRETCH_DEBS += $(SX_KERNEL)
mlnx-sdk-packages: $(addprefix $(DEBS_PATH)/, $(MLNX_SDK_BUILD_RDEBS) $(MLNX_SDK_RDEBS) $(PYTHON_SDK_API) $(SX_KERNEL))
